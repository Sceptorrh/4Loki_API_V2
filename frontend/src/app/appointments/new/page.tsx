...
export default function NewAppointmentPage() {
  ...
  useEffect(() => {
    ...
    if (startTime && selectedDogIds.length > 0 && Object.values(dogServices).some(services => services.length > 0)) {
      // Calculate the best time slot if we have dailyAppointments data
      if (dailyAppointments.length > 0) {
        findBestTimeSlot();
      } else {
        // If we don't have dailyAppointments data, just update the end time
        const totalDuration = getEstimatedDuration();
        
        console.log(`Total calculated duration from getEstimatedDuration: ${totalDuration} minutes`);
        
        // Set end time based on calculated total duration (which is already rounded to 15-min intervals)
  ...
  // Function to get estimated duration for selected services
  ...
  // Generate available times from 08:00 to 21:00 with 15-minute intervals
  ...
  const calculateBestTimeSlot = (appointments: DailyAppointment[], estimatedDuration: number = 60): Date => {
    ...
    if (availableSlots.length > 0) {
      ...
      console.log('Using best available slot:', bestSlot, 'as time:', format(startTime, 'HH:mm'));
      return startTime;
    }
    ...
    if (roundedLastEnd + estimatedDuration <= workdayEnd) {
      ...
      console.log('Using start time after last appointment:', format(startTime, 'HH:mm'));
      return startTime;
    }
  ...
  // Function to find the best time slot based on existing appointments
  const findBestTimeSlot = (appointments = dailyAppointments, force = false) => {
    ...
    const estimatedDuration = selectedDogIds.length > 0 ? getEstimatedDuration() : 60;
                    ...
                    <div>
                      ...
                      <DatePicker
                        ...
                        onChange={(time: Date) => {
                          ...
                          // Use the getEstimatedDuration function which now handles 15-min intervals correctly
                          const totalDuration = getEstimatedDuration();
